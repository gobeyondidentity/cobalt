...
// TestEncryptionWithAutoGeneratedKey tests behavior when SECURE_INFRA_KEY is not set.
// With auto-generation, a key will be created automatically, so encryption happens.
func TestEncryptionWithAutoGeneratedKey(t *testing.T) {
	// Use temp directory for auto-generated key
	tmpDir := t.TempDir()
	...
	// Disable insecure mode; with auto-gen, encryption should still work
	SetInsecureMode(false)
	defer SetInsecureMode(false)

	testData := []byte("sensitive-private-key-data")

	// With auto-generated key, encryption should work
	encrypted, err := EncryptPrivateKey(testData)
	if err != nil {
		t.Fatalf("EncryptPrivateKey failed: %v", err)
	}

	// Data should be encrypted (different from plaintext)
	if bytes.Equal(encrypted, testData) {
		t.Error("with auto-generated key, data should be encrypted")
	}
...
func TestIsEncryptionEnabled(t *testing.T) {
	...
	// Test with key set via env var
	os.Setenv("SECURE_INFRA_KEY", "test-key")
	if !IsEncryptionEnabled() {
		t.Error("expected encryption to be enabled with env var key")
	}

	// Test without env var; auto-generation kicks in
	os.Unsetenv("SECURE_INFRA_KEY")
	if !IsEncryptionEnabled() {
		t.Error("expected encryption to be enabled with auto-generated key")
	}
}
